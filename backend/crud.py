...
def get_video_by_project(db: Session, video_id: str, project_id: Optional[str]) -> Optional[models.Video]:
    if project_id is None:
        # If no project_id specified, just check by youtube_id
        return db.query(models.Video).filter(models.Video.youtube_id == video_id).first()
    else:
        try:
            ...
            # If project_id is not a valid UUID or None, return None
            return None

def get_videos(db: Session, skip: int = 0, limit: int = 100) -> List[models.Video]:
    return db.query(models.Video).offset(skip).limit(limit).all()

def get_videos_by_ids(db: Session, video_ids: Optional[List[str]]):
    if not video_ids:
        return []
    ids = [UUID(id) for id in video_ids]
    return db.query(models.Video).filter(models.Video.id.in_(ids)).all()
...
def update_video_summary(db: Session, video_id: str, summary: str, status: str = "completed") -> Optional[models.Video]:
    """
    Update video summary and processing status.
    ...
    if db_video:
        ...
        db_video.summary_processing_status = status
        
        # Update processed timestamp if status is completed or failed
...
def get_knowledge_items_by_video(db: Session, video_id: str) -> List[models.KnowledgeItem]:
    """Get all knowledge items for a specific video"""
    return db.query(models.KnowledgeItem).filter(models.KnowledgeItem.video_id == video_id).all()
...
def delete_project(db: Session, project_id: UUID) -> bool:
    """
    Delete a project and all its associated videos and knowledge items.
    ...
    # Delete knowledge items for each video
    for video in videos:
        knowledge_items = get_knowledge_items_by_video(db, str(video.id))
    ...
    # Delete knowledge items directly associated with the project (not through videos)
    knowledge_items = get_knowledge_items_by_project(db, project_id)
    for item in knowledge_items:
        db.delete(item)
...
def delete_video(db: Session, video_id: str) -> bool:
    """
    Delete a video and all its associated knowledge items.
    ...
    # Delete all knowledge items associated with this video
    knowledge_items = get_knowledge_items_by_video(db, video_id)
    for item in knowledge_items:
        db.delete(item)
